...
public class MissingReplyDao {
    public static final String SQL_MISSING_REPLY_SELECT = "SELECT * FROM MISSING_REPLY";
    public static final String SQL_MISSING_REPLY_INSERT = "INSERT INTO MISSING_REPLY(reply_Detail, reply_Create_Datetime,reply_Modify_Datetime, User_ID) VALUES (?,?,?,?)";
    public static final String SQL_MISSING_REPLY_UPDATE = "UPDATE MISSING_REPLY reply_Detail=?, reply_Modify_Datetime=? WHERE NO=?";
    public static final String SQL_MISSING_REPLY_DELETE = "DELETE FROM MISSING_REPLY WHERE NO=?";
    public static final String SQL_MISSING_REPLY_SELECT_ONE = "SELECT * MISSING_REPLY WHERE NO=?";
    ...
    // C
    public static void miReplyInput(ReplyDto replyDto) {
        try {
    ...
    // D
    public static void miReplyDelete(ReplyDto replyDto) {
        conn = JdbcUtil.getConnection();
        try {
            pstmt = conn.prepareStatement(SQL_MISSING_REPLY_DELETE);
    ...
    // U
    public static void miReplyModify(ReplyDto replyDto) {
        conn = JdbcUtil.getConnection(); //DB와 연결하기 위해 JdbcUtil의 getConncection 메소드 소환
        try {
            pstmt = conn.prepareStatement(SQL_MISSING_REPLY_UPDATE); // prepareStatement에 쿼리를 입력하여 DB를 불러와 Statement pstmt에 담는 구문
    ...
    // R
    public static List<ReplyDto> miReplySelectAll() {
        List<ReplyDto> list = new ArrayList<ReplyDto>();
        conn = JdbcUtil.getConnection();
        try {
            stmt = conn.createStatement();
            rs = stmt.executeQuery(SQL_MISSING_REPLY_SELECT);
            while (rs.next()) {
                int no = rs.getInt(1);
    ...
    // R
    public static ReplyDto miReplySelectOne(ReplyDto replyDto) { // one
        conn = JdbcUtil.getConnection();
        ReplyDto tmpdto = null;
        try {
            pstmt = conn.prepareStatement(SQL_MISSING_REPLY_SELECT_ONE);
            pstmt.setInt(1, replyDto.getNo());
